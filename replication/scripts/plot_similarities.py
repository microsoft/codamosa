...
colors = ["#544F7D","#695F77","#7F6E72","#947E6C","#AA8D67","#BF9D61","#D4AC5B","#EABC56","#FFCB50"]
...
def plot_cumulative_similarities(max_sim_list: List[float], proj_name = ""):
    ...
    if last_sim > cumulative_sims[-1][0]:
        cumulative_sims.append((last_sim, sims_under_current))
    if last_sim == 1:
        print(f"{proj_name}: {cumulative_sims[-1][1]-cumulative_sims[-2][1]} ({(100*(cumulative_sims[-1][1]-cumulative_sims[-2][1])/cumulative_sims[-1][1])} %) have similarity {last_sim}")
    ...
    if cumulative_sims[-1][0] - cumulative_sims[-2][0]> 0.1:
       ...
       cumulative_sims = cumulative_sims[:-2] + [(r, cumulative_sims[-2][1]) for r in np.arange(second_last,last,0.05)] + cumulative_sims[-1:]

    if proj_name == "":
        plt.plot([cs[0] for cs in cumulative_sims], [cs[1]/sims_under_current for cs in cumulative_sims])
    else:
        plt.plot([cs[0] for cs in cumulative_sims], [cs[1]/sims_under_current for cs in cumulative_sims], color=hl_color(proj_name,next(colorcycler)),label = proj_name.replace("python-",""), linestyle=next(linecycler), mfc="white",marker=next(markercycler), markevery=0.05, markersize=6, linewidth=hl_width(proj_name), drawstyle='steps-post', alpha=hl_op(proj_name), zorder=hl_zorder(proj_name))
    bot, top = plt.ylim()
    plt.ylim((0, top))
    ax = plt.gca()
...
def plot_edit_dist_similarities(similarities_map, proj_name_order, plot_output_dir):
    ...
    plt.ylabel("Cumulative Percent of Test Cases\nwith Maximum Similarity $\leq S_e$", fontsize=10)
...
def scatter_sizes(data):
    ...
    for proj in reversed(data):
        ...
        plt.scatter(similarities,lens, color=next(colorcycler),marker=next(markercycler), alpha=0.2, label=proj.replace('python-',''))
...
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    if len(sys.argv) !=3:
        print(f"usage: python3 {sys.argv[0]} pkl_file_with_similarity_analysis_data plots_output_dir")
        exit(1)
    ...
    sim_tuples = sorted(sim_tuples, reverse=True, key=lambda proj_sims: (proj_sims[1][-1], len([v for v in proj_sims[1] if v == proj_sims[1][-1]])/len(proj_sims[1])))
    extra_tuples =  [(p, (sims[-1], len([v for v in sims if v ==sims[-1]])/len(sims))) for p, sims in sim_tuples ]
