...
generation_stat_names = ['ParsableStatements', 'ParsedStatements', 'UninterpStatements', 'LLMStageSavedTests', 'LLMNeededUninterpreted', 'LLMNeededExpansion', 'LLMCalls', 'LLMQueryTime']
GenerationInfo = collections.namedtuple('GenerationInfo', ['parsable_stmts', 'parsed_stmts', 'uninterp_stmts', 'saved_tests', 'saved_uninterp_tests', 'saved_expansion_tests', 'llm_calls', 'llm_time'])
...
class CoverageContainer:
    ...
    def add_other_stat_for_module(self, module_name: str, config_name: str, stat_name: str, stat_data: Any):
        try:
        ...
        if config_name in self._other_stats_storage:
            if module_name in self._other_stats_storage[config_name]:
                if stat_name in self._other_stats_storage[config_name][module_name]:
                    self._other_stats_storage[config_name][module_name][stat_name].append(stat_data)
                else:
                    self._other_stats_storage[config_name][module_name][stat_name] = [stat_data]
            else:
                self._other_stats_storage[config_name][module_name] = {stat_name: [stat_data]}
        else:
            self._other_stats_storage[config_name]= {module_name: {stat_name: [stat_data]}}

    def add_observation_for_module(self, module_name: str, config_name: str, coverage_data: np.array):
        """
    ...
    def add_codamosa_generations_for_module(self, module_name: str, config_name: str, obs_data: np.array):
        """
    ...
    def test_for_significant_final_cov(self, module_name: str, configs = None):
        ...
        for config1 in configs:
            for config2 in configs:
                if config1 != config2:
                    observations1 = [o[-1] for o in self.get_observations(module_name, config1)]
                    observations2 = [o[-1] for o in  self.get_observations(module_name, config2)]
                    if len(observations1) > 0 and len(observations2) > 0:
                        u, p = scipy.stats.mannwhitneyu(observations1, observations2, alternative='greater')
                        if p < 0.05:
        ...
        for config1 in configs:
            for config2 in configs:
                if config1 != config2:
                    ...
                    observations2 = [o[-1] for o in  self.get_observations(module_name, config2)]
                    if len(observations1) > 0 and len(observations2) > 0:
                        u, p = scipy.stats.ttest_ind(observations1, observations2, equal_var=False, alternative='greater')
                        if p < 0.05:
                            print(f'[{module_name}] Found {config1} had signficiantly higher coverage than {config2} (p={p})')
                            output[(config1, config2)] = (u, p)
        return output
    ...
    def compare_with_scaled_query_time(self, module_name: str, scale: float):
        ...
        query_time = self.get_values_of_stat('LLMQueryTime', module_name, 'codamosa-0.8-uninterp')
    ...
    def get_average_final_coverage_by_config(self, module_name: str):
        ...
        for config in self.get_configs():
            final_observations = [o[-1] for o in self.get_observations(module_name, config)]
    ...
    def get_all_differences(self, config_1, config_2):
        ...
        common_keys = sorted(list(set(config_1_results.keys()).intersection(set(config_2_results.keys()))))
        differences = []
        ...
        for module_name in tqdm.tqdm(common_keys):
            ...
            signficant_results = self.test_for_significant({config_1: c1_results_at_1, config_2: c2_results_at_1})
            difference_in_means_at_t = np.mean([c1_val - c2_val for c1_val in c1_results_at_1 for c2_val in c2_results_at_1])#np.mean(c1_results_at_1) - np.mean(c2_results_at_1)
            #assert difference_in_means_at_t == 
            stds_in_means_at_t = np.std([c1_val - c2_val for c1_val in c1_results_at_1 for c2_val in c2_results_at_1])
            ...
            differences.append((difference_in_means_at_t, module_name, significance, stds_in_means_at_t))
    ...
    def get_mosa_increases_and_expansion_proportions(self):
        ...
        def avg_percentage(num, denom):
            pcts = [num[i]/denom[i] if isinstance(denom[i], int)  else 0 for i in range(len(denom))]
        ...
        for diff_in_means, module_name, sig, std_in_means in diffs:
            saved_llm_tests = self.get_values_of_stat("LLMStageSavedTests", module_name, CM)
            expansion_llm_tests = self.get_values_of_stat("LLMNeededExpansion", module_name, CM)
            uninterpreted_llm_tests = self.get_values_of_stat("LLMNeededUninterpreted", module_name, CM)
            uninterpreted_calls_llm_tests = self.get_values_of_stat("LLMNeededUninterpretedCallsOnly", module_name, CM)
            if sig != 'not significant':
               
               print(f"{module_name}: {diff_in_means}, expansion {avg_percentage(expansion_llm_tests,saved_llm_tests)}, uninterp {avg_percentage(uninterpreted_llm_tests, saved_llm_tests)}, uninterp calls {avg_percentage(uninterpreted_calls_llm_tests, saved_llm_tests)} ")
               ret_tuple= diff_in_means, avg_pct(expansion_llm_tests,saved_llm_tests), avg_pct(uninterpreted_llm_tests, saved_llm_tests), avg_pct(uninterpreted_calls_llm_tests, saved_llm_tests)
    ...
    def get_query_time_and_saved_tests(self, module, tech):
        ...
        """
        saved_tests = np.array([0 if val == "" else val for val in cc.get_values_of_stat("LLMStageSavedTests", module, tech)])
        query_time = np.array([0 if val == "" else val for val in cc.get_values_of_stat("LLMQueryTime", module, tech)])
        return np.mean(query_time), np.mean(saved_tests), np.mean([saved_tests[i]/query_time[i] for i in range(len(saved_tests)) if query_time[i] > 0])
    ...
    def cycle_through_stat_correlations(self, config_1, config_2):
        ...
        for stat_name in stat_names:
            ...
            for diff in diffs:
                ...
                config_1_stat_values = cc.get_values_of_stat(stat_name, module, config_1)
                config_2_stat_values = cc.get_values_of_stat(stat_name, module, config_2)
                print(config_1_stat_values)
                print(config_2_stat_values)
                stat_diffs = np.mean([(c1 - c2) for c1 in config_1_stat_values for c2 in config_2_stat_values])
        ...
        for i in range(len(configs_to_idx)):
            for j in range(len(configs_to_idx)):
                if j != len(configs_to_idx) - i - 1:
                    plt.text(j, i, s=int(winners_array[i, j]), color='black' if winners_array[i, j] < 50 else 'white')
        
        names_for_label = [get_nice_name(c) for c in configs_to_idx.keys()]

        plt.yticks(list(configs_to_idx.values()) , labels = names_for_label)
        plt.xticks(list(configs_to_idx.values()) , labels = list(reversed(names_for_label)),rotation=90)
        plt.title('Number of Benchmarks with Statistically Signifcant Coverage Increases')
    ...
    def plot_average_over_all_benchmarks(self, configs = None):
        ...
        for i, config in enumerate(configs):
           ...
           for module_name in common_keys:
               ...
               coverage_results.append(np.mean(np.array(self.get_observations(module_name, config)), axis = 0))
    ...
    def plot_relative_average_per_benchmark(self, relative_config, configs):
        common_modules = [m for m in self.get_modules() if all(m in self._storage[c] for c in configs + [relative_config])]

        all_differences: List[List[Tuple[float, float]]]= []
        for module_name in common_modules:
            base_results = [o[-1] for o in self.get_observations(module_name, relative_config)]
            diffs_for_module : List[Tuple[float, float]]= []
            for i, config in enumerate(configs):
                coverage_results = [o[-1] for o in self.get_observations(module_name, config)]
        ...
        for i, config in enumerate(configs):
            diffs_and_stds= [diff_and_std_lst[i] for diff_and_std_lst in all_differences]
            print(len(diffs_and_stds))
            plt.scatter(range(len(diffs_and_stds)), [diff for diff, std in diffs_and_stds], label=config)
    ...
    def multi_scatter_coverage(self, configs):
        ...
        common_keys = sorted([m for m in self.get_modules() if all(m in c for c in config_results)], key=lambda m: np.mean([l[-1] for l in config_results[0][m]]))
        plt.figure(figsize=(20,4))
        colors= [(68,119,170,0.3),(104,204,238,0.3),(34,136,51,0.3),(204,187,68,0.3),(238,102,119,0.3),(170,41,119,0.3),(187,187,187,0.3)]
        colors= [(68,119,170,0.3),(204,187,68,0.3),(238,102,119,0.3),(170,41,119,0.3),(104,204,238,0.3),(34,136,51,0.3),(187,187,187,0.3)]
    ...
    def violin_plot_coverage(self, configs):
        ...
        """
        config_results = [self._storage[config] for config in configs]
        all_common_keys = sorted([m for m in self.get_modules() if all(m in c for c in config_results)], key=lambda m: np.mean(config_results[1][m]))
        common_keys = []
        ...
        for i, config in enumerate(configs):
            ...
            labels.append((mpatches.Patch(color=(c[0],c[1],c[2],1)), get_nice_name(config)))
    ...
    def plot_all_differences_at_end(self, config_1, config_2, order_compared_to = None):
        ...
        color_map = {f'{config_1} better': c1_better_color, f'{config_2} better': c2_better_color, 'not significant': no_better_color}
        differences = self.get_all_differences(config_1, config_2)
        if order_compared_to is not None:
            module_order_comparatively =[d[1] for d in self.get_all_differences(config_1, order_compared_to)] 
            avail_modules = [d[1] for d in differences]
            module_to_diff = {d[1]:d for d in differences}
            differences = [module_to_diff[module] for module in module_order_comparatively if module in avail_modules]
        plt.axhline(0, color='grey',linestyle='dashed', linewidth=1,zorder=0)
        plt.scatter(range(len(differences)), [d[0] for d in differences], color=[color_map[d[2]] for d in differences], s =4, zorder=10)
        plt.xticks()
        plt.ylabel('Absolute difference in mean coverage at end')
        plt.xlabel("Modules ordered by differences in average coverage")
        plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0, decimals=0))
        for color in color_map.values():
            plt.fill_between(range(len(differences)), [d[0] - d[3] for d in differences], [d[0] + d[3] for d in differences], 
                where=[color_map[d[2]] == color for d in differences],
                color=color, alpha=0.2)


        legend_lines = [Line2D([0], [0], markerfacecolor=(c1_better_color), marker='o', color='w', markersize=8),
                        Line2D([0], [0], markerfacecolor=(c2_better_color), marker='o',color='w',  markersize=8),
                        Line2D([0], [0], markerfacecolor=(no_better_color),marker='o', color='w',  markersize=8)]

        plt.legend(legend_lines, [f'Significant: {config_1} > {config_2}', f'Significant: {config_1} < {config_2}', 'No significant difference'], loc='best')
    ...
    def scatter_plot_coverage(self, config_1, config_2, plot_significant = True, coverage_type='TOTAL'):
        """
        A comparative scatter plot of the coverage of config_1 and config_2.
        """
        config_1_results = self._storage[config_1]
        config_2_results = self._storage[config_2]
        common_keys = self.get_common_modules([config_1, config_2])# sorted(list(set(config_1_results.keys()).intersection(set(config_2_results.keys()))))
        ...
        for module_name in common_keys:
            if coverage_type == 'TOTAL':
                ...
                data_1 = self.get_values_of_stat( 'BranchCoverage', module_name,config_1,)
                data_2 = self.get_values_of_stat( 'BranchCoverage', module_name,config_2,)
            elif coverage_type == 'LINE':
                data_1 = self.get_values_of_stat( 'LineCoverage', module_name,config_1,)
                data_2 = self.get_values_of_stat( 'LineCoverage', module_name,config_2,)
            else:
            ...
            if plot_significant: 
                significant_diffs = self.test_for_significant({config_1: data_1, config_2: data_2})
                if (config_1, config_2) in significant_diffs:
        ...
        plt.scatter(c1_nosig,c2_nosig, marker='o', linewidths=0.5, facecolors= (1,1,1,0.6),edgecolors=not_sig_color(0.6), s=12, label='No significant difference')
        if len(c1_c1gtc2) > 0:
            plt.scatter(c1_c1gtc2,c2_c1gtc2, linewidth=1, marker=get_marker(config_1), color=get_color(config_1, 0.8), s=36, label=f'Significant: {get_nice_name(config_2)} < {get_nice_name(config_1).replace("CodaMOSA-","")}')
        if len(c1_c2gtc1) > 0:
            plt.scatter(c1_c2gtc1,c2_c2gtc1, linewidth=1, marker=get_marker(config_2), color=get_color(config_2, 0.9), s=36, label=f'Significant: {get_nice_name(config_2)} > {get_nice_name(config_1).replace("CodaMOSA-","")}')
    ...
    def plot_coverage_path_differences_highlight_significant(self, config_1, config_2):
        ...
        for module_name in tqdm.tqdm(common_keys):
            ...
            for t in range(end_time):
                ...
                signficant_results = self.test_for_significant({config_1: c1_results_at_1, config_2: c2_results_at_1})
                difference_in_means_at_t = np.mean(c1_results_at_1) - np.mean(c2_results_at_1)
                ...
                if cur_segment_type != last_segment_type and last_segment_type is not None:
                    if len(cur_segment) > 0:
                        cur_segment = append_current_segment(cur_segment, last_segment_type, difference_in_means_at_t)
                    else: 
        ...
        lc = mcollections.LineCollection(segments,  colors=colors, linewidth=1, linestyles=linestyles)

        def opaque(color):
            return (color[0], color[1], color[2], 1)

        legend_lines = [Line2D([0], [0], color=opaque(c1_better_color), linewidth=1, linestyle='solid'),
                        Line2D([0], [0], color=opaque(c2_better_color), linewidth=1, linestyle=(0, (5,1))),
                        Line2D([0], [0], color=opaque(no_better_color), linewidth=1, linestyle=(0, (1,3)))]
        ...
        plt.ylabel(f"Per-Benchmark Avg. Coverage Diff.\n{get_nice_name(config_1)} - {get_nice_name(config_2)}", fontsize=9.5)
        plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0, decimals=0))
        plt.legend(legend_lines, [
            f'Significant: {get_nice_name(config_1)} > {get_nice_name(config_2)}', 
            f'Significant: {get_nice_name(config_1)} < {get_nice_name(config_2)}', 
            'No significant diff.'], 
        prop={'size': 7.5}, ncol=2, bbox_to_anchor=(0.98,1.1))
        plt.tight_layout(pad=0.5,h_pad=0.5, w_pad=0.2)
    

def add_codamosa_generations_from_file(statistics_file: str, cc: CoverageContainer, module_name: str, config_name: str):
    """
    ...
    if os.path.isfile(statistics_file):
        with open(statistics_file, 'r') as f:
            ...
            for time in range(1, MAX_TIME + 1):
                cur_time = times[times_idx] if times_idx < len(times) else MAX_TIME +1000
                if time < cur_time:
            ...
            cc.add_codamosa_generations_for_module(module_name,config_name, interp_num_test_cases)

def add_coverage_from_file(statistics_file: str, cc: CoverageContainer, module_name: str, config_name: str):
    """
...
def add_other_stats_from_file(statistics_file: str, cc: CoverageContainer, module_name: str, config_name: str):
    """
    ...
    if os.path.isfile(statistics_file):
        with open(statistics_file, 'r') as f:
            ...
            if len(headers) != len(data):
                print(f"error in {statistics_file}, only {len(data)} data points")
                return
            other_stats = {headers[i]: data[i] for i in range(len(headers)) if not headers[i].startswith('CoverageTimeline_T')}
        ...
        if os.path.isdir(config_dir):
            ...
            for folder_name in tqdm.tqdm(os.listdir(config_dir)):
                ...
                if os.path.isdir(module_dir):
                    ...
                    add_coverage_from_file(statistics_file, cc, module_name, config_name)
                    add_other_stats_from_file(statistics_file, cc, module_name, config_name)
                    codamosa_timeline_file = os.path.join(module_dir, 'codamosa_timeline.csv')
                    add_codamosa_generations_from_file(codamosa_timeline_file, cc, module_name, config_name)
    return cc
...
def plot_main(cc: CoverageContainer):
    ...
    cc.plot_coverage_path_differences_highlight_significant('codamosa-0.8-uninterp', 'mosa')
    if PLOTS_DIR is not None:
        plt.savefig(os.path.join(PLOTS_DIR, f'vs_mosa_paths.pdf'))
    else:
        plt.show()
    print("=======Making Figure 2(b)========")
    cc.plot_coverage_path_differences_highlight_significant('codamosa-0.8-uninterp', 'codex-only')
    if PLOTS_DIR is not None:
        plt.savefig(os.path.join(PLOTS_DIR, f'vs_codex_paths.pdf'))

    else:
        plt.show()
    other_configs = [('mosa', 'a'), ('codex-only', 'b'), ('codamosa-0.8', 'c'), ('codamosa-0.2-uninterp', 'd'), ('codamosa-0.8-uninterp-no-targeting', 'e'), ('codamosa-0.8-uninterp-small', 'f')]
    for config_name, subfig_id in other_configs:
        ...
        if PLOTS_DIR is not None:
            plt.savefig(os.path.join(PLOTS_DIR, f'vs_{get_nice_name(config_name).replace("CodaMOSA-", "")}_scatter.pdf'))
        else:
            plt.show()
...
if __name__ == "__main__":
    ...
    if os.path.isdir(sys.argv[1]):
        ...
        print("Unsupported input method; either give directories as argument or a single pickle file")
        exit(1)
