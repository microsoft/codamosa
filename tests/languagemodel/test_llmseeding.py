...
def test_uninterpreted_statement_options():
    ...
    config.configuration.seeding.uninterpreted_statements = config.UninterpretedStatementUse.NONE
    test_cases = languagemodelseeding.get_targeted_testcase(gao_mock)
    assert len(test_cases) == 1
    test_case = test_cases[0]
    assert len(test_case.statements) == 4

    config.configuration.seeding.uninterpreted_statements = config.UninterpretedStatementUse.ONLY
    test_cases = languagemodelseeding.get_targeted_testcase(gao_mock)
    assert len(test_cases) == 1
    test_case = test_cases[0]
    assert len(test_case.statements) == 6

    config.configuration.seeding.uninterpreted_statements = config.UninterpretedStatementUse.BOTH
    test_cases = languagemodelseeding.get_targeted_testcase(gao_mock)
