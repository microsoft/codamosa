...
def test_can_retrieve_constructor_without_qualification():
    ...
    expandable_cluster : ExpandableTestCluster = TestClusterGenerator(
        'tests.fixtures.cluster.no_dependencies', True).generate_cluster()
...
def test_can_retrieve_typehint_constructor_imported_from():
    ...
    """
    expandable_cluster: ExpandableTestCluster = TestClusterGenerator(
        'tests.fixtures.cluster.typehint_constructor_imports', True).generate_cluster()

    # Check for `constructor`
    yat_noqualified = expandable_cluster.try_resolve_call('YetAnotherType')
    assert yat_noqualified.is_constructor() and yat_noqualified.owner.__name__ == 'YetAnotherType',\
        f'Wrong object: {yat_noqualified}'

    # Check for qualified `module.constructor`
    yat_qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.complex_dependency.YetAnotherType')
    assert yat_qualified.is_constructor() and yat_qualified.owner.__name__ == 'YetAnotherType',\
...
def test_can_retrieve_typehint_constructor_imported_as():
    ...
    assert aat_noqualified.is_constructor() and aat_noqualified.owner.__name__ == 'AnotherArgumentType',\
        f'Wrong object: {aat_noqualified}'
    # Check for qualified `module.constructor`
    aat_qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.another_dependency.AnotherArgumentType')
    assert aat_qualified.is_constructor() and aat_qualified.owner.__name__ == 'AnotherArgumentType',\
    ...
    assert aat_asqualified.is_constructor() and aat_asqualified.owner.__name__ == 'AnotherArgumentType',\
...
def test_can_retrieve_typehint_constructor_imported():
    ...
    assert sat_noqualified.is_constructor() and sat_noqualified.owner.__name__ == 'SomeArgumentType',\
        f'Wrong object: {sat_noqualified}'

    # Check for qualified `module.constructor`
    sat_qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.dependency.SomeArgumentType')
    assert sat_qualified.is_constructor() and sat_qualified.owner.__name__ == 'SomeArgumentType',\
...
def test_can_retrieve_constructor_imported_from():
    ...
    assert noqualified.is_constructor() and noqualified.owner.__name__ == 'Test',\
        f'Wrong object: {noqualified}'

    # Check for qualified `module.constructor`
    qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.no_dependencies.Test')
    assert qualified.is_constructor() and qualified.owner.__name__ == 'Test',\
...
def test_can_retrieve_constructor_imported_as():
    ...
    assert noqualified.is_constructor() and noqualified.owner.__name__ == 'AnotherArgumentType',\
        f'Wrong object: {noqualified}'

    # Check for qualified `module.constructor`
    qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.another_dependency.AnotherArgumentType')
    assert qualified.is_constructor() and qualified.owner.__name__ == 'AnotherArgumentType',\
        f'Wrong object: {qualified}'

    # Check for qualified `as_module.constructor`
    asqualified = expandable_cluster.try_resolve_call('ad.AnotherArgumentType')
    assert asqualified.is_constructor() and asqualified.owner.__name__ == 'AnotherArgumentType',\
...
def test_can_retrieve_constructor_imported():
    ...
    assert noqualified.is_constructor() and noqualified.owner.__name__ == 'SomeArgumentType',\
        f'Wrong object: {noqualified}'

    # Check for qualified `module.constructor`
    qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.dependency.SomeArgumentType')
    assert qualified.is_constructor() and qualified.owner.__name__ == 'SomeArgumentType',\
        f'Wrong object: {qualified}'
...
def test_can_retrieve_function_imported_from():
    ...
    assert noqualified.is_function() and noqualified.function_name == 'test_function', \
        f'Wrong object: {noqualified}'

    # Check for qualified `module.function`
    qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.no_dependencies.test_function')
    assert qualified.is_function() and qualified.function_name == 'test_function', \
        f'Wrong object: {qualified}'
...
def test_can_retrieve_function_imported_as():
    ...
    assert noqualified.is_function() and noqualified.function_name == 'a_function_to_call', \
        f'Wrong object: {noqualified}'

    # Check for qualified `module.function`
    qualified = expandable_cluster.try_resolve_call('tests.fixtures.cluster.another_dependency.a_function_to_call')
    assert qualified.is_function() and qualified.function_name == 'a_function_to_call', \
        f'Wrong object: {qualified}'

    # Check for qualified `as_module.function`
    asqualified = expandable_cluster.try_resolve_call('ad.a_function_to_call')
    assert asqualified.is_function() and asqualified.function_name == 'a_function_to_call', \
        f'Wrong object: {asqualified}'
...
def test_can_retrieve_function_imported():
    ...
    assert noqualified.is_function() and noqualified.function_name == 'method_with_optional', \
        f'Wrong object: {noqualified}'
...
def test_retrieve_gao_constructor_dependencies():
    ...
    gao = expandable_cluster.try_resolve_call('cd.SomeOtherType')
